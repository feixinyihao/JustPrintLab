...
@interface HQStepper ()<UITextFieldDelegate>

@property (strong, nonatomic) HQStepperButton *decreaseStepperButton; // Left HQStepperButton
@property (strong, nonatomic) UITextField     *valueTextField;		  // Center UITextField
@property (strong, nonatomic) HQStepperButton *increaseStepperButton; // Right HQStepperButton
...
+ (HQStepper *)stepperWithFrame:(CGRect)frame min:(CGFloat)min max:(CGFloat)max step:(CGFloat)step value:(CGFloat)value {
    HQStepper *stepper = [[HQStepper alloc] initWithFrame:frame];
...
- (void)buttonPressed:(HQStepperButton *)sender {
    if (![sender isEnabled]) {
        return;
    }
